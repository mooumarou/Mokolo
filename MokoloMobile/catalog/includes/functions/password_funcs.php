...
// salted or phpass password
  function mm_validate_password($plain, $encrypted) {
    if (mm_not_null($plain) && mm_not_null($encrypted)) {
      if (mm_password_type($encrypted) == 'salt') {
        return mm_validate_old_password($plain, $encrypted);
      }
...
// This function validates a plain text password with a
// salted password
  function mm_validate_old_password($plain, $encrypted) {
    if (mm_not_null($plain) && mm_not_null($encrypted)) {
...
// password.
  function mm_encrypt_password($plain) {
    if (!class_exists('PasswordHash')) {
      include(DIR_WS_CLASSES . 'passwordhash.php');
    }
...
// password.
  function mm_encrypt_old_password($plain) {
    $password = '';
...
// (phpass or salt)
  function mm_password_type($encrypted) {
